# server.py
# ВЕРСИЯ 37: Финальная, самая надежная версия с устойчивым подключением к БД
        ...
        async with database.connection() as connection:
            ...
            async with connection.transaction():
                ...
                if inviter_id:
                    ...
                    await connection.execute(users.update().where(users.c.id == inviter_id).values(points=users.c.points + 20000))
...
# ... (остальные функции остаются без изменений, но мы их оставим полными для ясности)
